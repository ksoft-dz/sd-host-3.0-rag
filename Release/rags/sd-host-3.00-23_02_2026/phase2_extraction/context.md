...
- **Extraction hints**: `extraction.tables.hints` — injects LLM prompt instructions for specific table patterns (e.g., register field format)
- **Table overrides**: `domain.registers.register_offsets[].table` — explicit table-to-register mapping when auto-matching fails
