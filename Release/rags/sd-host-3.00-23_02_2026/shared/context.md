...
| `pdf_utils.py` | PyMuPDF (fitz) wrappers: page text extraction, image rendering, multi-page image stitching (vertical concat), image downscaling for API limits (max 7500px). |
| `data_classes.py` | Node/Relation data classes mirroring v1 schema for backward compatibility. |
| `utils.py` | General helpers: JSON load/save, keyword extraction, offset normalization (`028h` → `028`), formatted printing (`print_step`, `print_banner`), duration formatting. |
