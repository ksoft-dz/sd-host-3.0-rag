...
def _parse_register_csv(csv_path: Path, reg_id: str,
    ...
    try:
        ...
        # If no bit_range column exists, every non-empty row is its own group.
        raw_rows = list(reader)
        groups = []
        if not bit_col:
            # No bit_range column: every row with a non-empty field_name is a field
            for row in raw_rows:
                check_col = name_col or desc_col or ""
                val = row.get(check_col, "").strip() if check_col else ""
                if val:
                    groups.append({"primary": row, "continuations": []})
        else:
            for row in raw_rows:
                bits = row.get(bit_col, "").strip()
                if bits:
                    groups.append({"primary": row, "continuations": []})
                elif groups:
