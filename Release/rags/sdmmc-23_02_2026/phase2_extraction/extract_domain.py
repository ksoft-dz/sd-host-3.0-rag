...
def _extract_registers(config: dict, skip_existing: bool, model: str, workers: int):
    ...
    for table in tables_map.get("tables", []):
        ...
        if "register" in title or "field descriptions" in title:
...
def _find_register_table(register_tables: List[dict], reg_name: str) -> Optional[dict]:
    ...
    def _normalize(s: str) -> str:
        """Strip spaces, underscores, hyphens for comparison."""
        return _re.sub(r'[\s_\-]', '', s.lower())
    
    reg_name_lower = reg_name.lower()
    main_name = reg_name_lower.replace(" register", "").strip()
    main_norm = _normalize(main_name)
    
    for table in register_tables:
        title_lower = table["title"].lower()
        # Exact prefix match: "BLOCKSIZE field descriptions" → prefix "blocksize"
        prefix = title_lower.replace("field descriptions", "").strip()
        if _normalize(prefix) == main_norm:
            return table
        # Substring match on original strings
        if main_name in title_lower:
            return table
        # Normalized substring match
        if main_norm in _normalize(title_lower):
            return table
    return None
...
def _parse_register_csv(csv_path: Path, reg_id: str,
    ...
    try:
        ...
        # Detect merged "bits fieldname" format:
        # When Field column has no separate bit_range, the field column
        # contains "3:0 SDMMC_DAT[7:4]" — split on first space.
        merged_bits_in_name = False
        if not bit_col and name_col:
            merged_bits_in_name = True
        ...
        groups = []
        for row in raw_rows:
            if merged_bits_in_name:
                # Extract bits from name column: "3:0 FIELDNAME" → bits="3:0"
                raw_name = row.get(name_col, "").strip()
                bits_match = re.match(r'^(\d+(?::\d+)?)\s+(.+)', raw_name)
                if bits_match:
        ...
        for i, group in enumerate(groups):
            ...
            elif merged_bits_in_name and name_col:
                # "bits fieldname" format: extract field name from name_col
                raw_name = row.get(name_col, "").strip()
                bits_match = re.match(r'^(\d+(?::\d+)?)\s+(.+)', raw_name)
                if bits_match:
                    field_name = bits_match.group(2).strip()
                else:
                    field_name = raw_name
            ...
            bits = row.get(bit_col, "").strip() if bit_col else ""
            if not bits and merged_bits_in_name and name_col:
                # Extract bits from merged name column
                raw_name = row.get(name_col, "").strip()
                bits_match = re.match(r'^(\d+(?::\d+)?)\s+', raw_name)
                if bits_match:
                    bits = bits_match.group(1)
...
def _extract_features(config: dict, skip_existing: bool, model: str, workers: int):
    ...
    for feature in features:
        ...
        try:
            desc = _enrich_feature(llm, feature, sections_context, config)
...
def _enrich_feature(llm: LLMClient, feature: dict, sections_context: str, config: dict = None) -> str:
    """Enrich a feature with LLM-generated description."""
    spec_name = config.get("spec", {}).get("name", "the hardware specification") if isinstance(config, dict) else "the hardware specification"
    system_prompt = f"""You are a hardware specification expert. Given a feature name and its groups, provide a concise 2-3 sentence description of what this feature does in the context of {spec_name}. Be specific and technical."""
