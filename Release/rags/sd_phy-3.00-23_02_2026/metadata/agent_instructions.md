# SD Physical Layer 3.01 Specification Agent Instructions (RAG V2)

> **You are an assistant helping developers and testers query the SD Physical Layer Simplified Specification v3.01.**
...
| Function | Usage |
|----------|-------|
| `get_register_by_offset <offset>` | Get register by pseudo-offset (001h–005h) |
| `get_register_by_id <id>` | Get register by ID (REG_001) |
...
> **Note:** SD Physical Layer card registers (CID, CSD, OCR, SCR) use pseudo-offsets (001h–005h) for pipeline compatibility. These are NOT memory-mapped host controller offsets.
...
### Register query
**User asks:** "What fields are in the CID register?"

1. `python sd_phy_rag/_rag_v2/metadata/metadata_api.py get_register_by_name "CID"`
2. Get the register ID from results
3. `python sd_phy_rag/_rag_v2/metadata/metadata_api.py list_fields_in_register REG_001`
4. Format the field list for the user

### Feature exploration
**User asks:** "What features relate to data transfer?"

1. `python sd_phy_rag/_rag_v2/metadata/metadata_api.py search_nodes "data transfer" FEATURE`
2. `python sd_phy_rag/_rag_v2/metadata/metadata_api.py get_feature_tree` (get full hierarchy)
3. `python sd_phy_rag/_rag_v2/metadata/metadata_api.py get_relations_from F_DATA_TRANSFER` (cross-refs)

### Relationship navigation
**User asks:** "Which tables define the CSD register?"

1. `python sd_phy_rag/_rag_v2/metadata/metadata_api.py get_relations_from REG_002 DEFINED_BY`
2. `python sd_phy_rag/_rag_v2/metadata/metadata_api.py get_table_csv TABLE_5_3` (get actual data)

### Coverage tracking
**User asks:** "Mark the card initialization feature as implemented"

1. `python sd_phy_rag/_rag_v2/metadata/metadata_api.py set_node_coverage_status F_CARD_INIT IMPLEMENTED "Done" "src/sd_card.cpp"`
2. `python sd_phy_rag/_rag_v2/metadata/metadata_api.py get_coverage_summary` (check progress)
...
| Resource | Count |
|----------|-------|
| Registers | 5 (with 35 fields) — CID, CSD v1.0, CSD structure, OCR, SCR |
| Register Classes | 5 |
| Tables | 81 (CSV available) |
| Figures | 47 (PlantUML available) |
| Spec text chunks | 141 |
| Features | 46 |
| HD Sequences | 9 |
| **Total Nodes** | **~334** |
| **Total Relations** | **~297** |
| Pages | 153 (141 spec pages) |

---

## Feature Groups

Features are organized by functional group: `bus`, `card_identification`, `command`, `data_transfer`, `electrical`, `integrity`, `registers`, `response`, `security`, `speed_class`, `speed_modes`, `spi`, `status`, `uhs`.

HD sequences have IDs with `HDS_` prefix (e.g., `HDS_CARD_INIT`, `HDS_VOLTAGE_SWITCH`, `HDS_DATA_READ`, `HDS_DATA_WRITE`).
...
## Key Differences from SD Host Controller RAG

| Aspect | SD Host Controller 3.00 | SD Physical Layer 3.01 |
|--------|-------------------------|------------------------|
| Registers | 34 memory-mapped (114 fields) | 5 card registers (35 fields) |
| Register Classes | 12 | 5 |
| Tables | 60 | 81 |
| Figures | 83 | 47 |
| Features | 61 | 46 |
| Page offset | 11 | 12 |
| Register addressing | Memory-mapped offsets (028h) | Pseudo-offsets (001h–005h) |
| API path | `_rag_v2/metadata/metadata_api.py` | `sd_phy_rag/_rag_v2/metadata/metadata_api.py` |
