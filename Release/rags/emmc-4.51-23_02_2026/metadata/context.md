...
The `emmc_rag/_rag_v2/metadata/metadata.json` is the unified knowledge graph produced by the config-driven RAG V2 pipeline. Generated through a clean, reproducible 3-phase pipeline driven by `spec_config.yaml`.

Configured for **Embedded Multimedia Card (e•MMC) Electrical Standard v4.51** (JESD84-B451, 264 PDF pages, 240 spec pages).
...
| Section | Content |
|---------|---------|
| `spec` | Name, version, PDF path, page_offset=22 |
...
| `domain.registers` | 5 classes, 6 card register offsets, 23 exclude_tables |
| `domain.features` | 59 feature definitions (55 in config + 4 auto-detected), 11 HD sequences |
...
```json
{
  ...
  "extraction_info": {
    ...
    "statistics": {
      "total_nodes": 586,
      "by_type": {
        "TABLE": 177, "FIGURE": 88, "SPEC_CHUNK": 241,
        "REGISTER": 5, "REG_CLASS": 5,
        "FEATURE": 59, "HD_SEQUENCE": 11
      }
    }
  },
  "nodes": [ /* ... */ ],
...
| Field | Description |
|-------|-------------|
| `id` | TABLE_N (single sequential number, e.g., TABLE_1, TABLE_82) |
| `type` | "TABLE" |
| `name` | Table title from spec |
| `description` | Table abstract |
| `source.page` | Spec page number |
| `source.pdf_page` | PDF page (spec_page + 22) |
...
| Field | Description |
|-------|-------------|
| `id` | FIG_N (single sequential number, e.g., FIG_1, FIG_88) |
| `name` | Figure title |
...
| `extras.section_number` | e.g., "6.6.1" |
...
| `name` | Register name (OCR, CID, CSD, Extended CSD, DSR) |
...
|-------|-------------|
| `id` | REGCLASS_NAME |
| `name` | Class name (Operation Conditions, Card Identification, Card Specific Data, Extended CSD, Driver Stage) |
| `extras.address_range` | start/end pseudo-offsets |

### FEATURE
| Field | Description |
|-------|-------------|
| `id` | F_NAME (e.g., F_CARD_INIT, F_BOOT_OPERATION) |
| `name` | Feature name |
| `extras.groups` | Feature groups (initialization, boot, partitioning, rpmb, etc.) |
...
| Field | Description |
|-------|-------------|
| `id` | HDS_NAME (e.g., HDS_DEVICE_INIT, HDS_BOOT_OPERATION) |
| `name` | Sequence name |
...
| Type | Source → Target | Count | Description |
|------|-----------------|-------|-------------|
| `REFERENCES` | CHUNK/FEATURE/HD_SEQ → TABLE/FIGURE | 399 | Cross-reference |
| `CHILD_OF` | SPEC_CHUNK → SPEC_CHUNK | 78 | Section hierarchy |
| `DEFINED_BY` | REGISTER → TABLE | 6 | Register fields from table |
| `BELONGS_TO` | REGISTER → REG_CLASS | 6 | Register classification |
| `PART_OF` | FEATURE → FEATURE | 31 | Sub-feature hierarchy |

## PAGE_OFFSET

```python
PAGE_OFFSET = 22  # PDF page = spec_page + 22
```

## Metadata API

**Script**: `emmc_rag/_rag_v2/metadata/metadata_api.py`
...
```python
api.set_node_coverage_status("F_CARD_INIT", "IMPLEMENTED", notes="Done", implemented_in="src/emmc.cpp")
